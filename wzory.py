...
def Gear_wheel_parameters(m, zx, k, x,alfa):
    """
    :param m: Moduł pary kół zębatych
    ...
    df = d - 2 * hf
    dfx = df + 2 * x * m
    db = mt.cos(mt.radians(20))* d
    dg_list = [ha,d,da,dax,hf,df,dfx,db]
    ...
    print("Średnica zasadnicza db = " + str(round(db, 3)) + " [mm]")
